...
def main():
    ...
    while not valid_inputs:
        try:
            ...
            if stop == "":
                stop = int(len(list_of_input_lines))+1
            else:
                stop=int(stop)
            if chunk_len == "":
                chukn_len=False
    ...
    if chunk_len:
        chunks = (stop-start)//chunk_len+1
    else:  
        chunks=1
        chunk_len=stop-start
    for i in range(chunks):
        out_f=open("Results//order_"+str(order)+"_lines_"+str(start+1+i*chunk_len)+"to"+str(min(start+1+(i+1)*chunk_len,stop-1))+".txt",'w')
        #note that stop is not included
        
        
        #for index in range(len([line[0] for line in list_of_input_lines[start:stop]])):
        for index in range(start+i*chunk_len,min(start+(i+1)*chunk_len-1,stop-1)):
            #print(index)
